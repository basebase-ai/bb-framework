...
export const schema = {
  [`${APP_ID}_pages_public`]: {
    fields: {
      ...
      createdBy: { type: "string", required: true, auto: true }, // Firebase Auth UID
      contributors: { type: "array", items: { type: "string" }, default: [] }, // Array of UIDs who edited
      createdAt: { type: "timestamp", auto: true },
      updatedAt: { type: "timestamp", auto: true },
    },

    indexes: [["slug"], ["createdAt"], ["title"]],

    rules: {
      // Pages are publicly readable
      read: "true",
      // Anyone authenticated can create a page
      create: "auth != null && request.resource.data.createdBy == auth.uid",
      // Anyone authenticated can edit any page (wiki-style)
      write: "auth != null",
      // Only creator can delete (for now)
      delete: "auth != null && auth.uid == resource.data.createdBy",
    },
  },
