...
export function DocumentReader({ documentId, onBack }) {
  const { data: doc, loading: docLoading } = useDocument(
  ...
  return (
    <Box style={{ display: "flex", gap: "1rem", height: "calc(100vh - 120px)" }}>
      {/* Main content area */}
            ...
            {tokens.map((token, index) => {
              ...
              return (
                <span
                  ...
                  onMouseEnter={(e) => {
                    e.currentTarget.style.background = "rgba(34, 139, 230, 0.2)";
      ...
      {showVocabPanel && (
        <Stack style={{ width: 280, flexShrink: 0 }} gap="sm">
          ...
          <Box style={{ flex: 1, minHeight: 0 }}>
            <VocabularyPanel onClose={() => setShowVocabPanel(false)} />
          </Box>
        </Stack>
      )}
