...
export function TableCell({ value, field }) {
  ...
  // Check for Firestore Timestamp (has toDate method or seconds/nanoseconds)
  const isTimestamp = value && typeof value === 'object' && (
    typeof value.toDate === 'function' ||
    (value.seconds !== undefined && value.nanoseconds !== undefined)
  );

  // Handle TIME_AGO field type (must check before generic object handler)
  if (field.fieldType === FIELD_TYPES.TIME_AGO || (isTimestamp && field.fieldType === 'timeAgo')) {
    const timestamp = value?.toDate?.() ?? (value instanceof Date ? value : new Date(value));
    const now = new Date();
  ...
  // Handle objects and arrays gracefully (but not Timestamps)
  if (typeof value === 'object' && !Array.isArray(value) && !isTimestamp && field.fieldType !== FIELD_TYPES.MULTI_SELECT) {
    return (
