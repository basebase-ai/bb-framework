...
import { Table, Container, Title, Text, Badge, Button, Group, Paper, Stack } from "@mantine/core";
...
export function AppsList() {
  ...
  const { data: apps, loading, error, add } = useCollection("apps", {
  ...
  const handleAddSample = async () => {
    try {
      await add({
        name: `Sample App ${Date.now()}`,
        description: "A sample application created from the UI",
        status: "draft",
        version: "0.1.0",
      });
    } catch (err) {
    ...
    <Container size="xl" py="xl">
      <Stack gap="md">
        <Group justify="space-between" align="center">
          ...
          <Button onClick={handleAddSample}>Add Sample App</Button>
        </Group>
